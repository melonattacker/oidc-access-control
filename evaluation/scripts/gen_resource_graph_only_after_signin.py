...
def plot_and_save_graph(labels, values, errors, y_label, filename, legend_labels):
    ...
    bars = plt.bar(labels, values, color=['lightgray', 'grey', 'dimgray'], yerr=errors, capsize=5)
...
for csv_file in [baseline_after_signin_csv, proposed_after_signin_csv, proposed_after_signin_confidential_csv]:
...
# CPU使用率のグラフを描画 & 保存
plot_and_save_graph(labels, cpu_values, cpu_errors, 'CPU使用率 [%]', './evaluation/graph/cpu_usage_comparison_only_after_signin.png', legend_labels)

# メモリ使用量のグラフを描画 & 保存
plot_and_save_graph(labels, mem_values, mem_errors, 'メモリ使用量 [MB]', './evaluation/graph/memory_usage_comparison_only_after_signin.png', legend_labels)