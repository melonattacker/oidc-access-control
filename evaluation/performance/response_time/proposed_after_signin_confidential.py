...
async def main():
    async with async_playwright() as p:
        ...
        # After sigin in confidential request (victim, victim's browser)
        for i in range(100):
            start_time = time.time() # start time

            await page.click('#afterLoginConfidentialRequestButton')
            await page.wait_for_selector('p#content:text("After sigin in confidential request succeeded")')
        ...
        if save_to_csv_flag:
            save_to_csv(response_times_after_sign_in, filename="./data/performance/response_time/proposed/response_times_after_signin_confidential.csv")
